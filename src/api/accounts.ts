...
/**
 * Calls your Supabase Edge Function to refresh the Monarch account.
 */
...
export async function getChaseBalance(): Promise<number> {
  const res = await fetch(`${FN}/chase-balance`, { method: 'GET' });
  if (!res.ok) {
    const err = await res.text();
...
/**
 * Calls your Supabase Edge Function to get the number of transactions awaiting review.
 */
